class PriceRange extends HTMLElement {
  constructor() {
    // Add this code in the FacetFiltersForm constructor
    // document.addEventListener(
    //   "theme:component:price_range",
    ...
    this.selectors = {
      inputMin: ".field__input--min",
      inputMax: ".field__input--max",
    ...
    this.controls.min.value = parseInt(this.controls.min.input.placeholder, 10);
    this.controls.max.value = parseInt(this.controls.max.input.placeholder, 10);
  ...
  setActiveControlValue(valueIn) {
    ...
    if (this.activeControl.input.value !== this.activeControl.value) {
      ...
      this.activeControl.input.dispatchEvent(
        new CustomEvent("theme:component:price_range", {
          bubbles: true,
          cancelable: false,
          detail: {
            sender: "theme:component:price_range",
            input: this.activeControl.input,
          },
        })
      );
    }
  ...
  bindEvents() {
    [this.controls.min, this.controls.max].forEach((item) => {
      ...
      item.input.addEventListener("change", this.onRangeChange.bind(this));
